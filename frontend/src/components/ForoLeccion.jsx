...
export const ForoLeccion = ({ leccionId }) => {
  ...
  // 1. Cargar historial y conectar WebSocket al montar el componente
  useEffect(() => {
    cargarHistorial();
    conectarWebSocket();
  ...
  const cargarHistorial = async () => {
    try {
      ...
      console.error("Error cargando el historial del foro:", error);
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 mt-8 flex flex-col h-[500px]">
      {/* HEADER DEL FORO */}
      <div className="p-4 border-b border-gray-200 bg-gray-50 rounded-t-xl flex justify-between items-center shrink-0">
      ...
      <div className="p-4 bg-white border-t border-gray-200 rounded-b-xl shrink-0">
        <form onSubmit={enviarMensaje} className="flex gap-3">
          <input
            ...
            disabled={!conectado}
            className="flex-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 outline-none transition-all"
          />
          <button
            type="submit"
